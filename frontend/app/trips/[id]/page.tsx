...
export default function TripDetailPage() {
  ...
  const shareWhatsApp = () => {
    if (!trip) return;
    const startFmt = new Date(trip.start_date + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' });
    const endFmt = new Date(trip.end_date + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' });
    const days = Math.ceil((new Date(trip.end_date + 'T12:00:00').getTime() - new Date(trip.start_date + 'T12:00:00').getTime()) / 86400000) + 1;
    let text = `🗺️ *Roteiro: ${trip.destination_city}*\n📅 ${startFmt} → ${endFmt} (${days} dias)\n\n`;
    if (itinerary.length > 0) {
      const byDay: Record<number, string[]> = {};
      for (const item of itinerary) {
        const attr = attractions.find(a => a.id === item.attraction_id);
        if (attr) byDay[item.day_number] = [...(byDay[item.day_number] || []), attr.name];
      }
      for (const [day, names] of Object.entries(byDay).sort((a, b) => Number(a[0]) - Number(b[0]))) {
        text += `*Dia ${day}:* ${names.join(', ')}\n`;
      }
      text += `\n`;
    }
  ...
  const hiddenGemsFor = (attr: Attraction): Attraction[] => {
    ...
    const inItinNames = new Set(
      itinerary
        .map(i => attractions.find(a => a.id === i.attraction_id)?.name?.toLowerCase().trim())
        .filter(Boolean) as string[]
    );
    const seen = new Set<string>();
    return attractions
      .filter(a => !inItin.has(a.id) && a.id !== attr.id && !inItinNames.has(a.name.toLowerCase().trim()))
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <CityImage city={trip.destination_city} className="relative h-72 bg-gradient-to-r from-brand-teal to-brand-teal-dark overflow-hidden">
        ...
        <div className="absolute top-4 right-6 flex gap-2 print:hidden">
          ...
          <button onClick={shareWhatsApp} title="Compartilhar no WhatsApp" className="bg-green-500/70 backdrop-blur-sm text-white px-3 py-1.5 rounded-full text-sm flex items-center gap-1.5 hover:bg-green-600/80 transition">
      ...
      {trip && (
        <CurrencyConverter countryCode={trip.destinations?.[0]?.country_code} />
      )}
